...
class Vote extends Component {
  ...
  handleVote(event) {
    let voteSecret = '0x'
      + ethjsAbi
        .soliditySHA3(['bool', 'uint'], [this.state.voteOption, this.state.salt])
        .toString('hex')
  ...
  render() {
    if (this.props.account.isAdmin && !this.props.campaign.isStopped) {
      return (
        <div className={'Vote mt-3 ' + this.props.campaign.approvalState}>
          <div>
            ...
            {this.props.campaign.approvalState === 'Commit' ? (
              <div>
                {' '}
                {this.props.campaign.numVoteSecrets}{' '}
                {this.props.campaign.numVoteSecrets === 1 ? 'vote has' : 'votes have'} been placed{' '}
              </div>
            ) : null}
            {this.props.campaign.approvalState === 'Reveal' ? (
              <div>
                {' '}
                {this.props.campaign.numVoteReveals}{' '}
                {this.props.campaign.numVoteReveals === 1 ? 'vote has' : 'votes have'} been revealed{' '}
              </div>
            ) : null}
          </div>

          {(this.props.campaign.approvalState === 'Commit' && !this.props.campaign.hasVoted)
          || (this.props.campaign.approvalState === 'Reveal' && !this.props.campaign.hasRevealed) ? (
            <div className="mt-3">
              <form>
                <div className="form-row">
                  ...
                  <div className="col-auto">
                    <div
                      ...
                      data-toggle="buttons"
                      onClick={this.handleChangeVoteOption}
                      disabled={this.props.campaign.isStopped}
                    >
                      <label className="btn btn-outline-success active" >
                        <input type="radio" name="voteOptions" id="approve" autoComplete="off" />
                        Approve
                      </label>
                      <label className="btn btn-outline-danger">
                        <input type="radio" name="voteOptions" id="reject" autoComplete="off" />
                        Reject
                      </label>
                    </div>
                  </div>
                  ...
                  {this.props.campaign.approvalState === 'Commit' ? (
                    <div className="col-auto">
                      <button
                        type="submit"
                        className={
                          'btn btn-outline-' + (this.state.voteOption ? 'success' : 'danger')
                        }
